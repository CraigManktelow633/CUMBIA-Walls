...
%                     SECTION AND MEMBER RESPONSE OF RC WALLS

%                 luis.montejo@upr.edu, arsalan_niroomandi@yahoo.co.nz
...
firstRowBoundary = 3;
HBZ  = MLR(firstRowBoundary, 1);          % boundary Elements length (mm) (from extreme compression 
                                          % to the centre of the last confined longitudinal bar)
nBZ  = sum(MLR(1:firstRowBoundary, 2));   % # bars in the boundary elements
db   = mean(MLR(1:firstRowBoundary, 3));  % longitudinal bar diameter (mm)
                                         
ncx  = 2;                                 % # legs transv. steel x_dir
ncy  = 3;                                 % # legs transv. steel y_dir
Dh   = 5.47;                                % transverse reinforcement diameter (mm) 
sh   = 75;                               % spacing of transverse reinforcement (mm)
     
% Shear reinforcement details

Ds      = 6;                                  % diameter of shear reinf. (mm)
s       = 150;                                 % spacing of shear steel (mm)*
ns      = 2;                                   % # legs shear reinforcement
...
confined   = 'mc';              %mc=mu if no transverse reinforcement
unconfined = 'mu';
rebar      = 'ks';
...
fpc     = 39.2;              % concrete compressive strength (MPa)
Ec      = 35200;                 % concrete modulus of elasticity (MPa) or
...
esu     = 0.0769;            % long. steel maximum strain (usually ~0.10-0.15)*
...
% Shear strength inforamtion
phiS    = 0.85;         % Strength reduction factor for shear                      
...
AstBZ = 0.25*nBZ*pi*db^2;                               % Boundary elemenets area of long steel
Hcore = H - 2*clb + Dh;                                 % Core length
...
switch lower(unconfined)
   case 'mu'
      [ecun,fcun] = manderunWALL(Ec,AstBZ,Dh,clb,sh,fpc,fyh,eco,esm,espall,HBZ,B,ncx,ncy,wi,dels);
   case 'mc'
      [ecun,fcun] = manderconfWALL(Ec,AstBZ,Dh,clb,sh,fpc,fyh,eco,esm,espall,HBZ,B,ncx,ncy,wi,dels);   
   otherwise
...
switch lower(confined)
   case 'mu'
      [ec,fc] = manderunWALL(Ec,AstBZ,Dh,clb,sh,fpc,fyh,eco,esm,espall,HBZ,B,ncx,ncy,wi,dels);
   case 'mc'
      [ec,fc] = manderconfWALL(Ec,AstBZ,Dh,clb,sh,fpc,fyh,eco,esm,espall,HBZ,B,ncx,ncy,wi,dels);    
   otherwise
...
yl  = sort([yl dcore H-dcore HBZ H-HBZ]);     % add layers to consider unconfined concrete
k = 1;
...
yc1 = [yl(find(yl>=dcore & yl<HBZ)) HBZ];                           % confined concrete layers Boundary 1
yc2 = [yl(find(yl>=H-HBZ & yl<H-dcore)) H-dcore];                   % confined concrete layers Boundary 1

Atc = [yl(1) diff(yl)]*B;                                           % total area of each layer

Atcc  = [diff(yc1)*Bcore diff(yc2)*Bcore];                          % total area of each conf. layer

k  = 1;
for i=1:length(yl)
    if yl(i)<=dcore || yl(i)>H-dcore || (yl(i)>HBZ && yl(i)<=H-HBZ)  
        conclay(i,:) = [Atc(i) 0];
    end
    if (yl(i)>dcore && yl(i)<=HBZ) || (yl(i)>H-HBZ && yl(i)<=H-dcore)
...
TransvSteelRatioX  = ncx*0.25*pi*(Dh^2)/(sh*(HBZ-clb+Dh));
TransvSteelRatioY  = ncy*0.25*pi*(Dh^2)/(sh*Bcore);
...
eqcurv = max((Mn/fyM)*fycurv,fycurv);              % yield curvature,
...
switch lower(bending)
    case 'single'
        Lp  = max(kkk*L   + 0.1*H + kLsp*fy*Dbl,2*kLsp*fy*Dbl);            % Plastic hinge length
        LBE = L;
    case 'double'
        Lp  = max(kkk*L/2 + 0.1*H + kLsp*fy*Dbl,2*kLsp*fy*Dbl);            % Plastic hinge length
        LBE = L/2;
    otherwise
...
if SectionCurvatureDuctility>4
    ...
    escc   = 3*((sh/diam(1))^(-2.5));             % allowable steel compression strain
...
rotb  = C0*(1+C1*roeff)*((1+C2*P/(AgBZ*fpc))^(-1))*(1+C3*LBE/H+C4*diam(1)*fy/HBZ);    % plastic rotation at the onset of bar buckling
...
ShearSteelRatioY = ns*0.25*pi*(Ds^2)/(s*B);

kscr1  = (0.25*ShearSteelRatioY*Es*(B/1000)*((H-dcore)/1000)/(0.25+10*ShearSteelRatioY))*1000;
...
% Alvarado et al. Buckling model

epsilonp = max(min(((11-sh/db)/150),0.06),0.02);
gammaL = H - 2*clb-db;
curvbuck = epsilonp/(gammaL/1000);

switch lower(bending)
    case 'single'
        ksg   = (G*As/L)/1000;
        kscrb  = (kscr1/L);
        displbuck = ((fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000+fycurv * (((L+Lsp(i))/1000)^2)/3 + ((curvbuck - fycurv) * (Lp/1000)*((L+Lsp(i)-0.5*Lp)/1000))*((((fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)/(fycurv * (((L+Lsp(i))/1000)^2)/3)+1);
    case 'double'
        ksg   = (G*As/(L/2))/1000;
        kscrb  = (kscr1/(L/2));
        displbuck = ((2*fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000+fycurv * (((L+2*Lsp(i))/1000)^2)/3 + (curvbuck - fycurv) * (Lp/1000)*((L+2*(Lsp(i)-0.5*Lp))/1000)*((((2*fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)/(fycurv * (((L+2*Lsp(i))/1000)^2)/3)+1);
end

forcebuck = interp1(displ, Force, displbuck);

% Out of plane (lateral) instability of walls

switch lower(reinforcement)
    case 'singly'
        betaw = 0.5;
        HBZ=H;
    case 'doubly'
...
rol = AstBZ/(B*(HBZ+clb));                                     %longitudinal reinforcement ratio in the end region of the wall

m = rol * fy / fpc;                                            %mechanical reinforcement ratio of the wall end region

etac = 0.5+1.18*m-(1.38*m^2+1.18*m)^0.5;                       %normalised critical out-of-plane displacement

switch lower(bending)
    case 'single'
        ksg   = (G*As/L)/1000;
        kscrgb  = (kscr1/L);
        Lpw = min(0.15*L,0.5*H);                                %wall effective plastic hinge length
        L0 = min((0.2*H+ 0.044*L),0.8*hn);                      %critical buckling length of wall
        displGB = (((fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)+fycurv * (((L+Lsp(i))/1000)^2)/3 + (2*min(fy/Es,0.002)*Lpw*L*(40*B^2*Es*betaw*etac/(7*L0^2*fy)-1)/H)/1000*((((fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)/(fycurv * (((L+Lsp(i))/1000)^2)/3)+1);      %displacement capacity at the onset of wall lateral instability
    case 'double'
        ksg   = (G*As/(L/2))/1000;
        kscrgb  = (kscr1/(L/2));
        Lpw = min(0.15*L/2,0.5*H);                              %wall effective plastic hinge length
        L0 = min((0.2*H+ 0.044*L/2),0.8*hn);                    %critical buckling length of wall
        displGB = (((2*fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)+fycurv * (((L+2*Lsp(i))/1000)^2)/3 + (2*min(fy/Es,0.002)*Lpw*L*(40*B^2*Es*betaw*etac/(7*L0^2*fy)-1)/H)/1000*((((2*fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)/(fycurv * (((L+2*Lsp(i))/1000)^2)/3)+1);   %displacement capacity at the onset of wall lateral instability
end
    
forcelGB = interp1(displ, Force, displGB);

% Shear Strength:

lh = H - cMn - (clb+db/2);                                       %horizontal projection of the crack length (mm)
thetacr =  max((45-7.5* L/H)*pi/180,30*pi/180);                  %average cracking angle
hcr = min(lh / (tan(thetacr)),L);                                %projected vertical height of the critical flexure-shear inclined crack (mm)
Vs  =(ns*0.25*pi*(Ds^2)*fys*hcr/s)/1000;                         %shear reinforcement truss shear-resisting mechanism (kN)
...
for i =1:length(Dductf)
    Vc(i) = alpha*beta*min(max(0.05,0.41-0.06*Dductf(i)),0.29)*0.8*(fpc^(1/2))*Agross/1000;
end
...
if displbuck<=displ(end)
    plot(displbuck, forcebuck,'k^', 'LineWidth',2, 'DisplayName',': buckling (Alv. et.al.)','MarkerEdgeColor','k','MarkerSize',10,'MarkerFaceColor','g')
end
if displGB<=displ(end)
    plot(displGB, forcelGB,'k>', 'LineWidth',2, 'DisplayName',': out of plane fail.','MarkerEdgeColor','k','MarkerSize',10,'MarkerFaceColor','g')
end
...
output = [coverstrain;corestrain;ejen;steelstrain;mom;curv;Force;displsh;displf;displ;V];
...
fprintf(fid, 'Cover\tCore\tN.A\tSteel\tMoment\tCurvature\tForce\tSh displ.\tFl displ.\tTotal displ.\tShear strength\n'); 
fprintf(fid, 'Strain\tStrain\t[mm]\tStrain\t[kN-m]\t[1/m]\t[kN]\t[m]\t[m]\t[m]\t[kN]\n'); 
fprintf(fid, '%1.5f\t%1.5f\t%4.2f\t%1.5f\t%8.2f\t%1.5f\t%8.2f\t%1.5f\t%1.5f\t%1.5f\t%8.2f\n',output);
fprintf(fid, ' \n');
...
fprintf(fid, '\n');
if displbuck<=displ(end)
    bucklDdAl = interp1(displ,Dduct,displbuck);
    failCuDuAl = interp1(displ,CuDu,displbuck);
    bucklcurvAl = interp1(CuDu,curv,failCuDuAl);
    bucklmomAl  = interp1(CuDu,mom,failCuDuAl);
    fprintf(fid,'Alvarado et al. buckling model:\n');
    fprintf(fid, '\nCurvature Ductility for Buckling:  %8.2f\n',failCuDuAl);
    fprintf(fid, 'Curvature at Buckling:  %3.5f m\n',bucklcurvAl); 
    fprintf(fid, 'Displacement Ductility at Buckling:  %8.2f\n',bucklDdAl);
    fprintf(fid, 'Displacement at Buckling:  %3.5f m\n',displbuck);
...
fprintf(fid, '\n');
if displGB<=displ(end)
    bucklDdGB = interp1(displ,Dduct,displGB);
    failCuDuGB = interp1(displ,CuDu,displGB);
    bucklcurvGB = interp1(CuDu,curv,failCuDuGB);
    bucklmomGB  = interp1(CuDu,mom,failCuDuGB);
    fprintf(fid,'Out of plane fail :\n');
    fprintf(fid, '\nCurvature Ductility for Buckling:  %8.2f\n',failCuDuGB);
    fprintf(fid, 'Curvature at Buckling:  %3.5f m\n',bucklcurvGB); 
    fprintf(fid, 'Displacement Ductility at Buckling:  %8.2f\n',bucklDdGB);
    fprintf(fid, 'Displacement at Buckling:  %3.5f m\n',displGB);
